...
class Smooth:
    ...
    def __init__(self, base_classifier: torch.nn.Module, num_classes: int, sigma: float, t: int, sample_correct_predictions: bool = True):
        """
    ...
    def certify(self, x: torch.tensor, n0: int, n: int, alpha: float, batch_size: int, label: int = None) -> (int, float):
        ...
        if label is not None and cAHat != label:
